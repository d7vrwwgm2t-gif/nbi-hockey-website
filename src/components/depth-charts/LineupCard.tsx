          ...
          <div className="space-y-2">
            {team.forwards.length > 0 ? (
              team.forwards.map((line) => (
                <div key={line.line} className="grid grid-cols-[42px_1fr_1fr_1fr] items-center gap-2">
                  <div className="text-sm font-bold text-yellow-300">L{line.line}</div>
          ...
          <div className="space-y-2">
            {team.defense.length > 0 ? (
              team.defense.map((pair) => (
                <div key={pair.pair} className="grid grid-cols-[42px_1fr_1fr] items-center gap-2">
                  <div className="text-sm font-bold text-yellow-300">P{pair.pair}</div>
            ...
            <div className="space-y-4">
              {team.powerPlay && team.powerPlay.length > 0 ? (
                team.powerPlay.map((unit) => (
                  <div key={unit.unit}>
                    <div className="mb-2 text-sm font-bold text-yellow-300">PP{unit.unit}</div>
                    <div className="grid gap-2 sm:grid-cols-2">
            ...
            <div className="space-y-4">
              {team.penaltyKill && team.penaltyKill.length > 0 ? (
                team.penaltyKill.map((unit) => (
                  <div key={unit.unit}>
                    <div className="mb-2 text-sm font-bold text-yellow-300">PK{unit.unit}</div>
                    <div className="grid gap-2 sm:grid-cols-2">
                      {unit.players.map((player) => (
                        <PlayerCard key={player.name} player={player} />
                      ))}
